...
/*!
*	\class GVector3
*	\brief 3차원 벡터를 표현하는 클래스 (n차원 벡터의 특수한 경우)
*
...
/*!
*	\brief 점들의 barycentric combination을 구한다
*
*	\param Points 점의 배열에 대한 포인터
*	\param Weights combination의 상수값
*	\param Size 점의 개수 (Points, Weights 배열의 크기)
*
*	\return 점들의 barycentric combination의 결과 (= Point[0] * Weight[0] + Points[1] * Weight[1] + ... + Points[Size-1] * Weight[Size-1])
*/
inline
...
protected:
	...
	int c;		// 열벡터의 차원
	double *M;	// 원소의 배열
	static double Precision;	// 오차의 한계
};
...
/*!
*	\brief 행렬의 grank (독립한 열벡터 혹은 행벡터의 수)를 구한다.
*
*	\param m 행렬
*
*	\return 구해진 행렬의 rank.
...
/*!
*	\brief	행렬 A를 하삼각행렬(lower triangle matrix) L과 상삼각행렬 (upper triangle matrix) U로 분해한다.\n
*			분해과정에서 부분피봇팅 및 행교환을 수행하지 않는다.
*
...
/*!
*	\brief	행렬 A를 하삼각행렬(lower triangle matrix) L과 상삼각행렬 (upper triangle matrix) U로 변환한다.\n
*			분해과정에서 부분피봇팅 (행교환)을 수행한다. 따라서, Idx = (0, 1, 2, ...)와 같지 않으면, L*U 는 A와 같지 않다.\n
*			분해 후, lin_sys_lu(A, x, b, Idx)를 호출하여 선형시스템 Ax = b 를 만족하는 벡터 x를 구한다.
*
*	\param A n행 n열의 행렬으로 함수호출 후, LU 형태로 변환된다.
*	\param Idx 행 벡터의 교환정보가 저장된다.
*	\param d 행 벡터의 교환정보가 짝수이면 1, 홀수이면 -1 이 저장된다.
*
*	\return 성공: true, 실패: false.
*/
inline 
...
/*!
*	\brief	행렬 A를 Householder 행렬을 사용하여 Q * R로 분해한다.
*
*	\param A m x n 행렬.
...
/*!
*	\brief	행렬 A를 singular value decomposition을 수행한다.\n
*			- A = U * W * tr(V) 로 분해된다.\n
*
*	\param A m x n 행렬 (m >= n)
*	\param U m x n 행렬로서 열벡터는 A의 열공간의 단위기저가 저장된다.
*	\param S 특이값이 저장될 n 차원벡터. W는 S의 원소로 생성되는 대각선 행렬. 
*	\param V n x n 행렬로서 열벡터는 A의 행공간 및 널공간의 단위기저가 저장된다.
*/
inline
...
/*!
*	\brief dcmp_sv 를 사용하여 주성분 (principal component)를 찾는다.
...
protected:
	// 데이터 멤버.
	GPoint3 p;
	GVector3 v;		// 생성되는 직선의 방정식: l(t) = p + v * t
	static double Precision;
};
...
protected:
	// 데이터 멤버
	GVector3 n;		// 평면의 방정식 ax + by + cz + d = 0 에서 n = (a, b, c)
	double d;		// n = (a, b, c), d = -n * p (p: 평면위의 임의의 점)
	static double Precision;
};
...
*	\param alpha barycentric 좌표
*	\param beta barycentric 좌표
*	\param bCull 직선이 삼각형의 뒷면과 만나는 경우를 제외하려면. (디폴트: true).
*
...
bool intersect_line_triangle(GPoint3 &q, const GLine &l, const GPoint3 &p1, const GPoint3 &p2, const GPoint3 &p3, double &t, double &alpha, double &beta, const bool bCull)
{
	...
	if (bCull)	// 직선이 삼각형의 뒷면과 만나는 경우는 제외.
	{
...
/*!
*	\class GTransf
*	\brief 강체변환 (rigid-body)을 나타내는 클래스.
*
*	\author 윤승현 (shyun@dongguk.edu)
*	\date 2001년 1월 1일.
*/
...
protected:
	...
	int m;					// 파라미터 공간의 차원
	int n;					// 데이터 공간의 차원
	RBFType type;			// Radial basis 함수의 타입
	GMatrix C;				// 보간식의 상수가 저장될 행렬
	vector<GVector> Params;	// 보간파라미터의 m-차원 벡터
	vector<GVector> Values;	// 보간데이터의 n-차원 벡터
};
